 ...
 *
 * Built, dropped, and rebuilt — and the reason it was dropped turned out to be
 * a bug in the shared decoder rather than in Maryland.
 *
 * Maryland prints its citations with an EN DASH, not a hyphen:
 * "&sect;2&ndash;201." The decoder's catch-all replaced any unrecognised named
 * entity with a space, so that became "2 201", and a check for section "2-201"
 * reported a statute that plainly exists as missing. That is the worst failure
 * shape available — an authoritative-sounding "no such law" about a real one —
 * and it was silent and general, not particular to Maryland, because dashes
 * are everywhere in statutory numbering.
 *
 * The decoder now keeps en/em dashes and smart quotes. Here, comparisons also
 * normalise dash forms, because a caller will type a hyphen and the page will
 * always answer with an en dash.
 *
 * Maryland URLs also need an article SHORT CODE (`gcr` for Criminal Law) that
 * the citation does not carry. The list is small and stable, so it is a table.
 */
...
async function pwFetch(url: string | URL, init?: RequestInit): Promise<Response> {
  return fetchWithTimeout(url, init ?? {}, 'Maryland Code');
}

const BASE = 'https://mgaleg.maryland.gov/mgawebsite/Laws/StatuteText';
const UA = 'pipeworx-mcp-maryland-code/1.0 (+https://pipeworx.io)';

const ARTICLES: Record<string, { code: string; name: string }> = {
  'criminal law': { code: 'gcr', name: 'Criminal Law' },
  'crim law': { code: 'gcr', name: 'Criminal Law' },
  'criminal procedure': { code: 'gcp', name: 'Criminal Procedure' },
  'transportation': { code: 'gtr', name: 'Transportation' },
  'family law': { code: 'gfl', name: 'Family Law' },
  'estates and trusts': { code: 'get', name: 'Estates and Trusts' },
  'labor and employment': { code: 'gle', name: 'Labor and Employment' },
  'courts and judicial proceedings': { code: 'gcj', name: 'Courts and Judicial Proceedings' },
  'tax general': { code: 'gtg', name: 'Tax - General' },
  'business regulation': { code: 'gbr', name: 'Business Regulation' },
  'real property': { code: 'grp', name: 'Real Property' },
  'health general': { code: 'ghg', name: 'Health - General' },
  'commercial law': { code: 'gcl', name: 'Commercial Law' },
  'state government': { code: 'gsg', name: 'State Government' },
};

/** Maryland writes en dashes where callers type hyphens. */
const flat = (s: string) => s.replace(/[‐-―−]/g, '-');

const tools: McpToolExport['tools'] = [
  {
    name: 'md_code_section',
    description:
    ...
    inputSchema: {
      type: 'object',
      properties: {
        article: { type: 'string', description: 'Maryland article name, e.g. "Criminal Law", "Transportation", "Family Law". Call md_articles for the list.' },
        section: { type: 'string', description: 'Section as cited, e.g. "2-201" or "21-902".' },
      },
  ...
  {
    name: 'md_articles',
    description: 'List the Maryland Code articles this pack can reach. Keyless. Use before md_code_section when you know the subject but not the article name.',
...
async function callTool(name: string, args: Record<string, unknown>): Promise<unknown> {
  if (name === 'md_articles') {
    ...
    for (const v of Object.values(ARTICLES)) if (!seen.has(v.code)) seen.set(v.code, v.name);
    return {
      returned: seen.size,
      articles: [...seen.entries()].map(([code, article]) => ({ article, code })),
      note: 'Maryland has more articles than these; this covers the ones people cite most. Ask and another can be added.',
      source: 'Maryland General Assembly',
    };
  }
  if (name !== 'md_code_section') throw new Error(`Unknown tool: ${name}`);

  const key = String(args.article ?? '').trim().toLowerCase().replace(/[.,]/g, '').replace(/\s+/g, ' ');
  const art = ARTICLES[key];
  const raw = flat(String(args.section ?? '').trim().replace(/\s+/g, ''));
  if (!art) {
    return { found: false, reason: 'unknown_article',
      message: `"${args.article}" is not an article this pack can reach.`,
      hint: 'Call md_articles for the supported list — "Criminal Law", "Transportation" and so on.' };
  }
  if (!/^\d+[A-Za-z]?-\d+[A-Za-z.]*$/.test(raw)) {
    return { found: false, reason: 'unparsed_section',
      message: `"${args.section}" is not a Maryland section citation.`,
      hint: 'Maryland sections look like "2-201" (murder) or "21-902" (DUI).' };
  }

  const url = `${BASE}?article=${art.code}&section=${encodeURIComponent(raw)}`;
  const res = await pwFetch(url, { headers: { 'User-Agent': UA } });
  if (!res.ok) throw new Error(`Maryland General Assembly returned HTTP ${res.status} for ${raw}`);
  const text = statuteText(await res.text());
  const norm = flat(text);

  // 200 is returned for unknown sections too, so require the section itself —
  // compared with dashes normalised, since the page uses en dashes throughout.
  const marker = `§${raw}`;
  const at = norm.indexOf(marker) !== -1 ? norm.indexOf(marker) : norm.indexOf(`§ ${raw}`);
  if (at === -1) {
    return { found: false, reason: 'section_not_found', article: art.name, section: raw,
      message: `No section ${raw} in the Maryland ${art.name} article. The site returns a page for unknown citations, so this is read from the content rather than the status code.`,
      hint: 'Section numbers are not contiguous — repealed sections are normal.' };
  }
  const body = norm.slice(at).trim();

  return {
    citation: `Md. Code, ${art.name} § ${raw}`,
    article: art.name, section: raw,
    text: body, text_chars: body.length, url,
    source: 'Maryland General Assembly (mgaleg.maryland.gov), keyless',
    attribution: 'Maryland statutes are public record.',
  };
}
